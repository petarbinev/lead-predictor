...
function update() {
  ...
  for (const key of ['prospects', 'leads', 'customers']) {
    const value = result.ok ? new Intl.NumberFormat(translation.locale).format(result.counts[key]) : '—';
    const count = document.querySelector(`#${key}-count`);
    count.textContent = value;
    count.classList.toggle('compact', value.length > 7);
    document.querySelector(`#${key}-percent`).textContent = result.ok ? `${new Intl.NumberFormat(translation.locale, { maximumFractionDigits: 1 }).format(result.shares[key])}%` : '—';
    document.querySelector(`#${key}-bar`).style.width = `${result.ok ? result.shares[key] : 0}%`;
  }
  document.querySelector('#campaign-goal').textContent = result.ok ? `${new Intl.NumberFormat(translation.locale, { style: 'currency', currency: currency.value }).format(result.revenue)} ${translation.t('goal')}` : '—';
  document.querySelectorAll('.currency-symbol').forEach(element => { element.textContent = currency.value === 'EUR' ? '€' : '$'; });
  currentPeriods = result.ok ? campaignTimeline(result.start, result.end, result.counts) : [];
  document.querySelector('#campaign-duration').textContent = result.ok ? `${currentPeriods.length} ${translation.t(currentPeriods.length === 1 ? 'monthOne' : 'monthMany')} · ${Math.round((result.end - result.start) / 86400000)} ${translation.t('days')}` : '—';
  renderChart(chartContainer, chartTable, currentPeriods, translation);
  clearTimeout(announcementTimer);
  announcementTimer = setTimeout(() => {
    document.querySelector('#results-announcement').textContent = result.ok ? ['prospects', 'leads', 'customers'].map(key => `${translation.t(key)}: ${new Intl.NumberFormat(translation.locale).format(result.counts[key])}`).join(', ') : errorSummary.textContent;
  }, 300);
}
...
language.addEventListener('change', () => { translation = translatePage(language.value); update(); });
