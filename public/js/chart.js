...
const defaults = { month: 'Month', people: 'People', prospects: 'Prospects', leads: 'Leads', customers: 'Customers', chartLabel: 'Cumulative campaign targets', chartInvalid: 'Complete the campaign details to see your timeline.' };

export function campaignTimeline(start, end, counts) {
  if (!(start instanceof Date) || !(end instanceof Date) || !Number.isFinite(+start) || !Number.isFinite(+end) || end <= start) return [];
  const periods = [];
  ...
  for (let month = 1; month <= 121; month++) {
    ...
    const cumulative = Object.fromEntries(keys.map(key => [key, Number((BigInt(counts[key]) * elapsed + duration - 1n) / duration)]));
...
export function renderChart(container, tableBody, periods, options = {}) {
  ...
  const step = Math.max(1, [1, 2, 5, 10].find(value => value * power >= rawStep) * power);
  const scaleMax = Math.ceil(max / step) * step;
  const scale = value => value / scaleMax * plotWidth;
  const svg = svgElement('svg', { viewBox: `0 0 ${width} ${height}`, width, height, role: 'group', 'aria-label': labels.chartLabel, class: 'campaign-chart' });
  svg.append(svgElement('text', { x: 2, y: 13, class: 'chart-axis-title' }, labels.month));
  for (let tick = 0; tick <= scaleMax; tick += step) {
    ...
    svg.append(svgElement('text', { x, y: bottom + 19, 'text-anchor': 'middle', class: 'chart-tick' }, compactNumber.format(tick)));
  }
  svg.append(svgElement('text', { x: left + plotWidth / 2, y: height - 4, 'text-anchor': 'middle', class: 'chart-axis-title' }, labels.people));
  ...
  periods.forEach((period, index) => {
    ...
    const group = svgElement('g', { tabindex: 0, role: 'button', 'aria-label': description, class: 'chart-row' });
    group.append(svgElement('rect', { x: 0, y, width, height: rowHeight, fill: 'transparent', class: 'chart-hit-area' }));
    group.append(svgElement('text', { x: 14, y: y + barHeight / 2 + 4, 'text-anchor': 'middle', class: 'chart-tick' }, period.month));
    for (const key of keys) group.append(svgElement('rect', { x: left, y: y + 2, width: scale(period.counts[key]), height: barHeight, rx: 2, class: `chart-bar chart-bar-${key}` }));
    const show = () => {
    ...
    const hide = () => { group.classList.remove('is-active'); tooltip.hidden = true; };
