...
export function calculateCampaign(input) {
  ...
  if (orderValue === null || orderValue === 0n) errors.orderValue = 'invalidOrder';
  if (!Number.isInteger(leadRate) || leadRate < 1 || leadRate > 100) errors.leadRate = 'invalidRate';
  if (!Number.isInteger(prospectRate) || prospectRate < 1 || prospectRate > 100) errors.prospectRate = 'invalidRate';
  ...
  if (prospects > MAX_SAFE) return { ok: false, errors: { revenue: 'tooLarge' } };

  const counts = { prospects: Number(prospects), leads: Number(leads), customers: Number(customers) };
  const shares = Object.fromEntries(Object.entries(counts).map(([key, count]) => [key, prospects ? count / counts.prospects * 100 : 0]));
  return { ok: true, counts, shares, start, end, revenue: Number(revenue) / 100 };
}
